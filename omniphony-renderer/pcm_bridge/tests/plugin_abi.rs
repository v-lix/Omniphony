...
/// Where cargo left the cdylib for this test run, rebuilding it from source.
///
...
fn the_built_plugin_loads_and_decodes_over_the_abi() {
    ...
    assert!(bridge.configure(RStr::from("input_codec"), RStr::from("dts")));
    ...
    assert!(!bridge.is_ready());
    assert_eq!(bridge.source_family().as_str(), "dts");
    let mut second = header(&[25, 26], 48_000, 1);
    ...
    assert_eq!(frame.sampling_frequency, 48_000);
    assert_eq!(frame.channel_labels.as_slice(), &[Lh, Rh]);
