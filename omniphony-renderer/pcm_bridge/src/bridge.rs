...
//! recognises a plain channel bed (`Engine::render_frame` treats any
...
pub(crate) struct PcmBridge {
    ...
    frames_emitted: u64,
    /// The original codec's placement family, configured before input arrives.
    /// PCM transport alone does not turn a Dolby or DTS bed into generic PCM.
...
impl FormatBridge for PcmBridge {
    ...
    fn configure(&mut self, key: RStr<'_>, value: RStr<'_>) -> bool {
        match key.as_str() {
            // There is only one presentation in host-decoded PCM.
            "presentation" => true,
            "input_codec" => {
                self.source_family = match value.as_str() {
                    "ac3" | "eac3" | "truehd" => "dolby",
                    "dts" => "dts",
                    _ => "pcm",
                };
                true
    ...
    fn fixed_channel_poses(&self) -> RVec<RChannelPose> {
        // OPCM declares speaker labels, not angles. Use the renderer's
        // placement policy for the source family without inventing poses.
        RVec::new()
    }

    fn source_family(&self) -> RString {
        RString::from(self.source_family)
    }
}
...
mod tests {
    ...
    #[test]
    fn accepts_presentation_and_rejects_unknown_options() {
        let mut bridge = PcmBridge::new(true);
    ...
    #[test]
    fn source_family_follows_the_original_codec_and_survives_a_seek() {
        let mut bridge = PcmBridge::new(false);
        assert_eq!(bridge.source_family().as_str(), "pcm");
        for (codec, family) in [
            ("ac3", "dolby"),
            ("eac3", "dolby"),
            ("truehd", "dolby"),
            ...
            assert!(bridge.configure(RStr::from("input_codec"), RStr::from(codec)));
            assert_eq!(bridge.source_family().as_str(), family);
            assert!(bridge.fixed_channel_poses().is_empty());
            let mut stream = header(&[0, 1], 48_000, 0);
            stream.extend(i32_frames(&[vec![1, 2]]));
            assert_eq!(push(&mut bridge, &stream).frames.len(), 1);
            assert!(bridge.is_ready());
            bridge.reset();
            assert!(!bridge.is_ready());
            assert_eq!(bridge.source_family().as_str(), family);
